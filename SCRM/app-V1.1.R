...
page_02 <- fluidPage(
  ...
  downloadBttn("dwnldbtwn_results_comparison_counts", label = "Download Table", color = "success", size = "xs"),
  h5("Regulatory Testing Counts"),
  DT::dataTableOutput("results_lot_table_regulatory_tests"),
  downloadBttn("dwnldbtwn_results_comparison_regulatory_tests", label = "Download Table", color = "success", size = "xs")
)
...
server <- function(input, output, session) {
  ...
  output$dwnldbtwn_results_comparison_counts <- downloadHandler(
    filename = function() {
      paste0(as.character(paste0(format(Sys.Date()),"-", format(Sys.time(), "%H-%M-%S"))), " comparison_table_counts.csv")
    },
    content = function(con) {
  ...
  output$dwnldbtwn_results_comparison_regulatory_tests <- downloadHandler(
    filename = function() {
      paste0(as.character(paste0(format(Sys.Date()),"-", format(Sys.time(), "%H-%M-%S"))), " comparison_table_regulatory_tests.csv")
    },
    content = function(con) {
  ...
  observeEvent(input$btn_run, {
    ...
    if (nrow(steps) == 0) {
      showNotification(id = "ntfcn_error_no_steps", "Error! No Steps are currently enabled.", duration = 10, closeButton = TRUE, type = "error")
    } else {
